...
export function useWizard<T extends WizardData>(
    ...
    const saveDraft = useCallback(async (): Promise<string | null> => {
        ...
        setIsSaving(true);
        try {
            const draftId = await onSaveDraft(data, currentStep);
            setHasDraft(true);
    ...
    const loadDraft = useCallback(async (draftId: string): Promise<boolean> => {
        setIsLoading(true);
        try {
            const result = await WizardPersistence.loadDraft<T>(draftId);
            if (result.success && result.data) {
                const draftData = result.data;
                setData(draftData.data);
                const stepIndex = config.steps.findIndex(step => step.id === draftData.currentStep);
                if (stepIndex !== -1) {
                    setCurrentStepIndex(stepIndex);
                }
                setHasDraft(true);
            ...
            const message = err instanceof Error ? err.message : "Error al cargar borrador";
            setError(message);
            toast.error(message);
            return false;
        } finally {
    ...
    }, [canComplete, onComplete, data, validateAllSteps, hasDraft, draftId, deleteDraft]);
