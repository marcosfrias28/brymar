...
export function useCreateContactInfo() {
    return useCreateMutation(
        async (data: CreateContactInfoData) => {
            ...
            if (!result.success) {
                throw new Error(result.error || 'Error al crear la información de contacto');
            }
            return result.data;
        },
        {
...
export function useUpdateContactInfo() {
    ...
    return useUpdateMutation(
        async (data: UpdateContactInfoData) => {
            ...
            if (!result.success) {
                throw new Error(result.error || 'Error al actualizar la información de contacto');
            }
            return result.data;
        },
        {
            ...
            onMutate: async (variables) => {
                ...
                const previousList = queryClient.getQueryData<ContactInfo[]>(listQueryKey);
                const previousDetail = queryClient.getQueryData<ContactInfo>(detailQueryKey);
...
export function useDeleteContactInfo() {
    ...
    return useDeleteMutation(
        async (data: { id: number }) => {
            ...
            if (!result.success) {
                throw new Error(result.error || 'Error al eliminar la información de contacto');
            }
            return result.data;
        },
        {
...
export function useContactInfoMutations() {
    ...
    return {
        ...
        // Convenience methods
        isLoading: createContactInfo.isPending || updateContactInfo.isPending || deleteContactInfo.isPending,
        isError: createContactInfo.isError || updateContactInfo.isError || deleteContactInfo.isError,
        error: createContactInfo.error || updateContactInfo.error || deleteContactInfo.error,
