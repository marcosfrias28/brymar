 ...
 */
export interface BaseQueryOptions<T> extends Omit<UseQueryOptions<T>, 'queryKey' | 'queryFn'> {
...
export function useBaseQuery<T>(
    ...
    const { notifyError, notifyLoading, notifySuccess, dismiss } = useNotificationContext();
    ...
    }, [query.isLoading, query.isRefetching, showLoadingToast, loadingMessage, notifyLoading, dismiss]);
    ...
    useEffect(() => {
        if (showSuccessToast && query.isSuccess && query.isRefetching === false && query.data) {
            notifySuccess(successMessage);
        }
    }, [query.isSuccess, query.isRefetching, query.data, showSuccessToast, successMessage, notifySuccess]);
