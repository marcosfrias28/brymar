...
export async function checkServiceHealth(): Promise<{
    healthy: boolean;
    services: Record<string, { status: 'healthy' | 'unhealthy'; latency?: number; error?: string }>;
}> {
    const config = getDeploymentConfig();
    const services: Record<string, { status: 'healthy' | 'unhealthy'; latency?: number; error?: string }> = {};
    ...
    try {
        ...
        services.ai = { status: 'unhealthy', error: error instanceof Error ? error.message : 'Unknown error' };
    }
    ...
    try {
        ...
        services.storage = { status: 'unhealthy', error: error instanceof Error ? error.message : 'Unknown error' };
    }
