...
async function resetDatabase() {
	...
	if (!connectionString) {
		...
		console.error(
			"   Configura una de estas variables de entorno para continuar."
		);
		process.exit(1);
	}
    ...
    `;
		...
		for (const sequence of sequences) {
			try {
				...
				console.log(
					`   ❌ Error eliminando secuencia ${sequence}: ${error.message}`
				);
			}
		}
...
async function main() {
	...
	if (isProduction && !forceReset) {
		console.error(
			"❌ Error: No se puede resetear la base de datos en producción"
		);
