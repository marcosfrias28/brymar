...
function debugSearchFilters() {
	...
	if (minPrice) {
		currentFilters.minPrice = Number.parseInt(minPrice, 10);
	}
	if (maxPrice) {
		currentFilters.maxPrice = Number.parseInt(maxPrice, 10);
	}
	if (bedrooms) {
		currentFilters.bedrooms = bedrooms;
	}
	if (bathrooms) {
		currentFilters.bathrooms = bathrooms;
	}
	if (minArea) {
		currentFilters.minArea = Number.parseInt(minArea, 10);
	}
	if (maxArea) {
		currentFilters.maxArea = Number.parseInt(maxArea, 10);
	}
	if (amenities) {
		currentFilters.amenities = amenities.split(",");
	}
	if (sortBy) {
	...
	console.log("\n✅ Suggested Fixes:");
	console.log(
		"1. Update SearchPropertiesInput.fromFormData to handle single values"
	);
	console.log(
		"2. Or update OptimizedSearchPage to send correct parameter names"
	);
