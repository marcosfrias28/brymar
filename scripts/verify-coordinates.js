...
async function verifyCoordinates() {
    ...
    `;
		...
		landsWithCoords.forEach((land) => {
			...
			console.log(
				`   - ${land.name}: ${coords.latitude.toFixed(4)}, ${coords.longitude.toFixed(4)}`
			);
		});
    ...
    `;
		...
		propertiesWithCoords.forEach((property) => {
			...
			const coords = location.coordinates;
			console.log(
				`   - ${property.title}: ${coords.latitude.toFixed(4)}, ${coords.longitude.toFixed(4)}`
			);
		});
    ...
    `;
		...
		if (landPercentage === "100.0" && propertyPercentage === "100.0") {
			...
			console.log(
				"💡 Ejecuta el script add-coordinates-simple.js para completar las coordenadas faltantes."
			);
		}
	} catch (error) {
