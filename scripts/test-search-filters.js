...
async function testSearchFilters() {
    try {
        ...
        apartmentResult.rows.forEach(property => {
            console.log(`  - ID: ${property.id}, Title: ${property.title}, Type: ${property.type}, Bathrooms: ${property.bathrooms}`);
        });
        ...
        console.log(`Found ${bathroomResult.rows.length} properties with 4+ bathrooms:`);
        ...
        console.log(`Found ${combinedResult.rows.length} apartments with 4+ bathrooms:`);
