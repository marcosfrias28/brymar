...
async function forceResetDatabase() {
	...
	if (!connectionString) {
		console.error(
			"❌ Error: No se encontró POSTGRES_URL_NON_POOLING o POSTGRES_URL"
		);
		process.exit(1);
	}
	...
	console.log(
		`🔗 Usando: ${connectionString.includes("pooler") ? "Conexión pooled" : "Conexión directa"}\n`
	);
	...
	try {
		...
		const remainingTables = remainingTablesResult.rows.map(
			(row) => row.tablename
		);
		...
		try {
			...
			for (const sequence of sequences) {
				try {
					...
					console.log(
						`   ❌ Error con secuencia ${sequence}: ${error.message}`
					);
				}
			}
		} catch (_error) {
			console.log(
				"   ⚠️  Error accediendo a secuencias (esto es normal en algunas bases de datos)"
			);
		}
		...
		if (error.message.includes("permission denied")) {
			...
			console.error(
				"   - El script hizo lo mejor posible con los permisos disponibles"
			);
			console.error(
				"   - Intenta ejecutar db:push para ver si el schema se aplica correctamente"
			);
		}
...
async function main() {
	...
	if (!forceReset) {
		...
		console.log(
			"   - Diseñado para bases de datos administradas con permisos limitados"
		);
		console.log(
			"   - Puede no eliminar todas las tablas debido a restricciones de permisos"
		);
		console.log(
