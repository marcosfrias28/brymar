...
// Types
export type SignedUrlOperation = typeof SIGNED_URL_CONFIG.ALLOWED_OPERATIONS[number];
...
export async function generateSignedUrl(
    ...
    const defaultExpiry = SIGNED_URL_CONFIG.DEFAULT_EXPIRY[operation] || SIGNED_URL_CONFIG.DEFAULT_EXPIRY.upload;
    const maxExpiry = SIGNED_URL_CONFIG.MAX_EXPIRY[operation] || SIGNED_URL_CONFIG.MAX_EXPIRY.upload;
    ...
    const token = generateSecureToken(filename, operation, expires, userId, metadata);
    ...
    const restrictions = {
        ...
        allowedMimeTypes: allowedMimeTypes || SIGNED_URL_CONFIG.ALLOWED_FILE_TYPES[fileType],
        operation,
    };
 ...
 */
function validateSignedUrlInputs(filename: string, options: SignedUrlOptions): void {
    // Validate filename
    ...
    if (options.expiresIn !== undefined) {
        ...
        const maxExpiry = SIGNED_URL_CONFIG.MAX_EXPIRY[options.operation] || SIGNED_URL_CONFIG.MAX_EXPIRY.upload;
    ...
    if (options.fileType) {
        ...
        if (options.allowedMimeTypes) {
            const invalidTypes = options.allowedMimeTypes.filter(type => !allowedTypes.includes(type));
            if (invalidTypes.length > 0) {
                throw new ValidationError(
                    { allowedMimeTypes: [`Tipos MIME no válidos: ${invalidTypes.join(', ')}`] },
    ...
    if (options.maxSize !== undefined) {
        ...
        const absoluteMaxSize = Math.max(...Object.values(SIGNED_URL_CONFIG.SIZE_LIMITS));
...
function generateSecureToken(
    ...
    if (!secret || secret === 'fallback-secret') {
        console.warn('Using fallback secret for signed URLs. Set SIGNED_URL_SECRET in production.');
    }
