 ...
 */
export function validateFileExtension(filename: string, fileType: 'image' | 'video'): void {
 ...
 */
export function validateMimeType(mimeType: string, fileType: 'image' | 'video'): void {
 ...
 */
export function validateFileSize(size: number, fileType: 'image' | 'video'): void {
 ...
 */
export function validateFileCount(count: number, fileType: 'images' | 'videos'): void {
            ...
            try {
                ...
                if (file.type.startsWith('image/')) {
                    if (!isValidImageSignature(bytes, file.type)) {
                        reject(new ValidationError(
                            { signature: ['La firma del archivo no coincide con el tipo declarado'] },
...
export function validateImageUrl(url: string): void {
    try {
        ...
        const isTrustedDomain = UPLOAD_SECURITY_CONFIG.TRUSTED_DOMAINS.some(domain =>
 ...
 */
export function generateSecureFilename(originalFilename: string, fileType: 'image' | 'video'): string {
