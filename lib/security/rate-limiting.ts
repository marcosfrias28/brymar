...
class RateLimitStore {
    ...
    get(key: string): { count: number; resetTime: number; requests: number[] } | undefined {
        return this.store.get(key);
    }

    set(key: string, value: { count: number; resetTime: number; requests: number[] }): void {
...
export class RateLimiter {
    ...
    async isAllowed(
        ...
        const shouldSkip = (success === true && this.skipSuccessfulRequests) ||
...
export async function getRateLimitStatus(identifier: string): Promise<{
    ...
    const [aiGeneration, imageUpload, draftSave, formSubmission] = await Promise.all([
...
export class AdaptiveRateLimiter extends RateLimiter {
    ...
    async isAllowed(identifier: string, success?: boolean): Promise<{
        allowed: boolean;
        remaining: number;
        resetTime: number;
        totalHits: number;
        ...
        if (suspicious) {
            ...
            console.warn(`Suspicious rate limit activity detected for ${identifier}:`, {
                totalHits: result.totalHits,
                remaining: result.remaining,
                usageRatio,
            });
