...
export class AriaValidator {
    ...
    static validateAriaAttributes(element: HTMLElement): {
        ...
        const hasProperRole = interactiveTags.includes(tagName) ||
            Boolean(role && ['button', 'link', 'menuitem', 'tab', 'option'].includes(role));
...
export class KeyboardNavigationTester {
    ...
    static testFocusTrap(container: HTMLElement): {
        ...
        if (focusableElements.length === 1) {
            issues.push('Only one focusable element - focus trap may not work properly');
        }
...
export class AccessibilityAuditor {
    ...
    static auditContainer(container: HTMLElement = document.body): {
        ...
        const elementAudits = Array.from(interactiveElements).map(el => {
            const audit = this.auditElement(el as HTMLElement);
            return {
                element: el,
        ...
        const passedElements = elementAudits.filter(audit => audit.score >= 80).length;
        const failedElements = totalElements - passedElements;

        const overallScore = totalElements > 0
            ? elementAudits.reduce((sum, audit) => sum + audit.score, 0) / totalElements
            : 100;
...
export const runAccessibilityCheck = (element?: HTMLElement) => {
    ...
    if (audit.elementAudits.some(a => a.score < 80)) {
        ...
        audit.elementAudits
            .filter(a => a.score < 80)
            .forEach(audit => {
                console.log(`${audit.tagName}: ${audit.score.toFixed(1)}% - ${audit.issues.join(', ')}`);
            });
        console.groupEnd();
    }
