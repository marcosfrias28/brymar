...
import { WizardError, AIServiceError, UploadError, NetworkError } from '../errors/wizard-errors';
    ...
    for (let attempt = 1; attempt <= options.maxAttempts; attempt++) {
        try {
            ...
            const shouldRetry = options.shouldRetry
                ...
                : lastError instanceof NetworkError || (lastError instanceof WizardError && lastError.retryable);
...
export async function retryAIOperation<T>(
    ...
    throw result.error || new AIServiceError(
...
export async function retryUploadOperation<T>(
    ...
    throw result.error || new UploadError(
...
export async function retryNetworkRequest<T>(
    ...
    throw result.error || new NetworkError(
        'Network request failed after all retry attempts'
    );
}
...
export async function retryDraftOperation<T>(
    ...
    throw result.error || new WizardError(
