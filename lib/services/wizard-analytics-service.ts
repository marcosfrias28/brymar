...
class WizardAnalyticsService {
    ...
    trackStepCompletion(stepNumber: number, timeSpent: number, data: Record<string, any> = {}): void {
        this.trackEvent(WizardEventType.STEP_COMPLETED, {
            timeSpent,
            ...data
        }, stepNumber);
    }
    ...
    getSessionSummary(): {
        ...
        const stepCompletions = this.events.filter(e => e.eventType === WizardEventType.STEP_COMPLETED);
        const aiAttempts = this.events.filter(e => e.eventType === WizardEventType.AI_GENERATION_REQUESTED);
        const aiSuccesses = this.events.filter(e => e.eventType === WizardEventType.AI_GENERATION_SUCCESS);
        const uploadAttempts = this.events.filter(e => e.eventType === WizardEventType.UPLOAD_STARTED);
        const uploadSuccesses = this.events.filter(e => e.eventType === WizardEventType.UPLOAD_SUCCESS);
        const errors = this.events.filter(e => e.eventType === WizardEventType.ERROR_OCCURRED);
