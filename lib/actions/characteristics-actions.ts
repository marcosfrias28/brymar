 ...
 */
export async function getCharacteristics(propertyType?: PropertyType): Promise<PropertyCharacteristic[]> {
    try {
        let query = db
            .select({
        ...
        const filteredResults = propertyType
            ? results.filter(char => {
                ...
                return charPropertyTypes.length === 0 || charPropertyTypes.includes(propertyType);
            })
            : results;
 ...
 */
export async function getDraftCharacteristics(draftId: string): Promise<PropertyCharacteristic[]> {
    try {
        const results = await db
            ...
            .innerJoin(
                propertyCharacteristics,
                eq(propertyDraftCharacteristics.characteristicId, propertyCharacteristics.id)
            )
...
export async function saveDraftCharacteristics(
    ...
    try {
        ...
        if (selectedCharacteristics.length > 0) {
            ...
            await db.insert(propertyDraftCharacteristics).values(draftCharacteristics);
        }
 ...
 */
export async function deleteCustomCharacteristic(characteristicId: string): Promise<void> {
    try {
        const { headers } = await import('next/headers');
        const sessionData = await auth.api.getSession({
            headers: await headers(),
...
export async function getCharacteristicsStats(): Promise<{
    ...
    try {
        ...
        const byCategory = characteristics.reduce((acc, char) => {
            acc[char.category] = (acc[char.category] || 0) + 1;
            return acc;
        }, {} as Record<string, number>);

        const byPropertyType = characteristics.reduce((acc, char) => {
