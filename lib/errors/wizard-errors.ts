...
export class AIServiceError extends WizardError {
    constructor(
        ...
        const userMessages = {
            RATE_LIMIT: 'El servicio de IA está temporalmente ocupado. Inténtalo en unos minutos.',
...
export class UploadError extends WizardError {
    constructor(
        message: string,
        code: 'FILE_TOO_LARGE' | 'INVALID_TYPE' | 'UPLOAD_FAILED' | 'NETWORK_ERROR' | 'QUOTA_EXCEEDED' | 'PROCESSING_ERROR' | 'SECURITY_VALIDATION_FAILED',
        retryable: boolean = false,
        context?: Record<string, any>
    ) {
        const userMessages = {
            FILE_TOO_LARGE: 'El archivo es demasiado grande. El tamaño máximo permitido es 10MB.',
            INVALID_TYPE: 'Tipo de archivo no permitido. Solo se aceptan imágenes JPG, PNG y WebP.',
            UPLOAD_FAILED: 'Error al subir el archivo. Inténtalo nuevamente.',
            NETWORK_ERROR: 'Error de conexión durante la subida. Verifica tu conexión.',
            QUOTA_EXCEEDED: 'Se ha alcanzado el límite de almacenamiento.',
            PROCESSING_ERROR: 'Error al procesar la imagen. Inténtalo con otra imagen.',
            SECURITY_VALIDATION_FAILED: 'El archivo no pasó las validaciones de seguridad.'
        };
...
export class ValidationError extends WizardError {
    ...
    constructor(
        ...
        super(message, 'VALIDATION_ERROR', false, 'Por favor corrige los errores en el formulario.', context);
...
export class MapServiceError extends WizardError {
    constructor(
        message: string,
        code: 'GEOCODING_FAILED' | 'COORDINATES_OUT_OF_BOUNDS' | 'NETWORK_ERROR' | 'SERVICE_UNAVAILABLE',
        retryable: boolean = false,
        context?: Record<string, any>
    ) {
        const userMessages = {
            GEOCODING_FAILED: 'No se pudo obtener la ubicación. Verifica la dirección ingresada.',
            COORDINATES_OUT_OF_BOUNDS: 'La ubicación debe estar dentro de República Dominicana.',
            NETWORK_ERROR: 'Error de conexión con el servicio de mapas.',
            SERVICE_UNAVAILABLE: 'El servicio de mapas no está disponible temporalmente.'
        };
...
export class DraftServiceError extends WizardError {
    constructor(
        message: string,
        code: 'SAVE_FAILED' | 'LOAD_FAILED' | 'DELETE_FAILED' | 'NOT_FOUND' | 'PERMISSION_DENIED',
        retryable: boolean = false,
        context?: Record<string, any>
    ) {
        const userMessages = {
...
export class ErrorFactory {
    static createFromResponse(response: Response, context?: Record<string, any>): WizardError {
    ...
    static createFromError(error: unknown, context?: Record<string, any>): WizardError {
