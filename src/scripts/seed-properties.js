...
// Database connection
const connectionString = 'postgres://default:VAblR0TOeiF4@ep-hidden-lake-a4rxpjzx-pooler.us-east-1.aws.neon.tech:5432/verceldb?sslmode=require';
...
async function seedProperties() {
    try {
        ...
        // Insert properties using raw SQL
        for (const property of sampleProperties) {
            const result = await client`
        INSERT INTO properties (
          title, description, price, type, bedrooms, bathrooms, 
          area, location, status, featured, images, created_at, updated_at
        ) VALUES (
          ${property.title}, ${property.description}, ${property.price}, 
          ${property.type}, ${property.bedrooms}, ${property.bathrooms},
          ${property.area}, ${property.location}, ${property.status}, 
          ${property.featured}, ${property.images}, NOW(), NOW()
        ) RETURNING id, title, price
      `;

            console.log(`✅ Insertada: ${result[0].title} - $${result[0].price.toLocaleString()}`);
        }

        console.log('\n🎉 Seed completado exitosamente!');
        console.log(`📊 Total de propiedades insertadas: ${sampleProperties.length}`);
