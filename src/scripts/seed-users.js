    ...
    try {
        ...
        for (const user of sampleUsers) {
            try {
                ...
                const roleIcon = user.role === 'admin' ? '👑' : user.role === 'agent' ? '🏠' : '👤';
                console.log(`${roleIcon} ✅ ${user.name} (${user.role}) - ${user.email}`);
                insertedCount++;
        ...
        console.log(`📊 Total insertados: ${insertedCount}/${sampleUsers.length}\n`);
