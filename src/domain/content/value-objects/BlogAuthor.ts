...
export class BlogAuthor {
    ...
    private validate(author: string): void {
        if (!author || author.trim().length === 0) {
            throw new ValueObjectValidationError("Blog author cannot be empty");
        }

        const trimmedAuthor = author.trim();

        if (trimmedAuthor.length < BlogAuthor.MIN_LENGTH) {
            throw new ValueObjectValidationError(`Blog author must be at least ${BlogAuthor.MIN_LENGTH} characters long`);
        }

        if (trimmedAuthor.length > BlogAuthor.MAX_LENGTH) {
            throw new ValueObjectValidationError(`Blog author cannot exceed ${BlogAuthor.MAX_LENGTH} characters`);
        }
        ...
        if (!validNamePattern.test(trimmedAuthor)) {
            throw new ValueObjectValidationError("Blog author contains invalid characters");
        }
    }
    ...
    getInitials(): string {
        const names = this._value.trim().split(/\s+/);
        return names
    ...
    equals(other: BlogAuthor): boolean {
        return this._value.trim().toLowerCase() === other._value.trim().toLowerCase();
    }
