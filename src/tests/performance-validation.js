...
class PerformanceValidator {
    ...
    async measureBundleSize() {
        ...
        try {
            ...
            if (fs.existsSync(nextDir)) {
                ...
                console.log(`📊 Bundle size: ${this.formatBytes(this.results.bundleSize.total)}`);
            }
    ...
    measureMemoryUsage() {
        ...
        console.log(`   Heap Total: ${this.results.memoryUsage.formatted.heapTotal}`);
    ...
    countFiles() {
        ...
        for (const dir of directories) {
            ...
            if (fs.existsSync(dirPath)) {
                ...
                console.log(`   ${dir}: ${count.total} files (${count.ts} TS, ${count.tsx} TSX, ${count.js} JS)`);
            }
        }
    }
    ...
    async testApplicationPerformance() {
        ...
        try {
            ...
            console.log(`   Startup Time: ${performanceMetrics.startupTime.toFixed(0)}ms`);
            console.log(`   First Contentful Paint: ${performanceMetrics.firstContentfulPaint.toFixed(0)}ms`);
            console.log(`   Largest Contentful Paint: ${performanceMetrics.largestContentfulPaint.toFixed(0)}ms`);
            console.log(`   Time to Interactive: ${performanceMetrics.timeToInteractive.toFixed(0)}ms`);
    ...
    async validateDatabasePerformance() {
        ...
        try {
            ...
            console.log(`   Connection Time: ${dbMetrics.connectionTime.toFixed(0)}ms`);
            console.log(`   Simple Query: ${dbMetrics.simpleQuery.toFixed(0)}ms`);
            console.log(`   Complex Query: ${dbMetrics.complexQuery.toFixed(0)}ms`);
            console.log(`   Insert Operation: ${dbMetrics.insertOperation.toFixed(0)}ms`);
            console.log(`   Update Operation: ${dbMetrics.updateOperation.toFixed(0)}ms`);
    ...
    generateComparisonReport() {
        ...
        console.log(`   Bundle Size: ${comparison.bundleSizeReduction.toFixed(1)}% reduction`);
        console.log(`   Build Time: ${comparison.buildTimeImprovement.toFixed(1)}% improvement`);
        console.log(`   File Count: ${comparison.fileCountReduction.toFixed(1)}% reduction`);
        console.log(`   Startup Time: ${comparison.performanceImprovement.startupTime.toFixed(1)}% improvement`);
        console.log(`   First Contentful Paint: ${comparison.performanceImprovement.firstContentfulPaint.toFixed(1)}% improvement`);
    }
     ...
     */
    saveResults() {
        const resultsPath = path.join(process.cwd(), 'performance-validation-results.json');
    ...
    async runAll() {
        ...
        console.log('📊 Check performance-validation-results.json for detailed metrics');
