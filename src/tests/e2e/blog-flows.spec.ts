...
test.describe('Blog Management Flows', () => {
        ...
        await page.evaluate(() => {
            ...
            localStorage.setItem('auth-session', JSON.stringify({
                user: {
                    ...
                    role: 'admin',
                    permissions: ['canManageBlogs', 'canCreateBlogs', 'canEditBlogs', 'canPublishBlogs']
                }
            }));
        });
    });
    ...
    test('should handle blog post creation flow (admin/agent)', async ({ page }) => {
        ...
        if (await contentInput.isVisible()) {
            await contentInput.fill('This is a test blog post created by E2E tests. It contains sample content for testing purposes.');
        }
    ...
    test('should handle blog post editing flow', async ({ page }) => {
        ...
        const editElements = [
            ...
            page.locator('svg[class*="edit"]').locator('..') // Parent of edit icon
        ];
    ...
    test('should handle blog post publishing flow', async ({ page }) => {
        ...
        for (const indicator of publishIndicators) {
            try {
                ...
                if (indicator.textContent?.includes('Confirm') || indicator.textContent?.includes('Confirmar')) {
                    const confirmButton = page.locator('button:has-text("Confirm")').or(page.locator('button:has-text("Confirmar")'));
...
test.describe('Blog Content Management', () => {
        ...
        await page.evaluate(() => {
            ...
            localStorage.setItem('auth-session', JSON.stringify({
                user: {
                    ...
                    email: 'admin@example.com',
                    role: 'admin',
                    permissions: ['canManageBlogs', 'canCreateBlogs', 'canPublishBlogs']
                }
            }));
        });
    });
    ...
    test('should handle rich text editor', async ({ page }) => {
        ...
        if (editor) {
            ...
            await editor.fill('This is test content with **bold text** and *italic text*.');
