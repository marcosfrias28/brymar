...
test.describe('Critical User Flows - Simplified Architecture', () => {
        ...
        await page.evaluate(() => {
            ...
            localStorage.setItem('auth-session', JSON.stringify({
                user: {
                    ...
                    role: 'admin',
                    permissions: ['canManageProperties', 'canCreateProperties', 'canEditProperties', 'canManageBlog']
                }
            }));
        });
    });
    ...
    test('Complete Property Management Flow', async ({ page }) => {
        ...
        const createButton = page.locator('button:has-text("Add Property")').or(
        ...
        if (await createButton.isVisible()) {
            ...
            await page.fill('textarea[name="description"]', 'This property was created by E2E tests');
            ...
            await expect(page.locator('text=Property created').or(
        ...
        await expect(page.locator('[data-testid="property-card"]').or(
        ...
        if (await searchInput.isVisible()) {
            ...
            await expect(page.locator('text=E2E Test Property')).toBeVisible({ timeout: 5000 });
        }

        // 5. View property details
        const propertyLink = page.locator('text=E2E Test Property').or(
    ...
    test('Blog Management Flow', async ({ page }) => {
        ...
        const createButton = page.locator('button:has-text("Add Post")').or(
        ...
        if (await createButton.isVisible()) {
            ...
            const contentEditor = page.locator('textarea[name="content"]').or(
                page.locator('[data-testid="content-editor"]')
            ).first();

            if (await contentEditor.isVisible()) {
                await contentEditor.fill('This is a test blog post created by E2E tests. It contains sample content to verify the blog functionality works correctly.');
            }
            ...
            // Verify creation success
            await expect(page.locator('text=Post created').or(
                page.locator('text=Success')
            )).toBeVisible({ timeout: 10000 });
        }
        ...
        await expect(page.locator('[data-testid="blog-card"]').or(
        ...
        if (await publishButton.isVisible()) {
            ...
            await expect(page.locator('text=Published').or(
    ...
    test('Search and Filter Flow', async ({ page }) => {
        ...
        if (!await page.locator('input[name="search"]').isVisible({ timeout: 2000 })) {
            await page.goto('/properties');
        }
        ...
        if (await locationSelect.isVisible()) {
            ...
            const validOptions = options.filter(option => option.trim() && option !== 'Select...');
    ...
    test('Wizard Flow', async ({ page }) => {
        ...
        if (await descriptionInput.isVisible()) {
            await descriptionInput.fill('This item was created using the wizard in E2E tests');
        }
    ...
    test('Performance and Loading Flow', async ({ page }) => {
        ...
        await page.waitForSelector('main, [data-testid="main-content"], .main-content', { timeout: 10000 });
        ...
        for (const element of paginationElements) {
            if (await element.isVisible()) {
                const initialCount = await page.locator('[data-testid="property-card"]').or(
                    page.locator('.property-card')
                ).count();
                ...
                const newCount = await page.locator('[data-testid="property-card"]').or(
                    page.locator('.property-card')
                ).count();
