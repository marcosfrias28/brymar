...
function getLogLevel(): LogLevel {
  ...
  switch (envLevel) {
    ...
    default:
      // Valor por defecto basado en el entorno
      return isProduction ? LogLevel.WARNING : LogLevel.DEBUG;
  }
}
...
export async function error(message: string, errorObj?: any, context?: any): Promise<void> {
...
export async function critical(message: string, errorObj?: any, context?: any): Promise<void> {
  const sanitizedError = errorObj ? sanitizeError(errorObj) : '';
  const fullMessage = sanitizedError ? `${message}: ${sanitizedError}` : message;
  logMessage(LogLevel.CRITICAL, fullMessage, context);
}

// Note: Individual logger functions are exported above
// Use: import { info, error, debug, warning, critical } from './logger'

// Utilidades para mensajes de usuario
export const UserMessages = {
  EMAIL_SEND_ERROR: 'No se pudo enviar el código de verificación. Por favor, inténtalo de nuevo.',
  ...
  SYSTEM_ERROR: 'Ha ocurrido un error del sistema. Por favor, inténtalo más tarde.',
  NETWORK_ERROR: 'Error de conexión. Por favor, verifica tu conexión a internet.',
  VALIDATION_ERROR: 'Los datos proporcionados no son válidos.',
} as const;

// Función para obtener mensaje de usuario seguro
export async function getSafeUserMessage(errorType: keyof typeof UserMessages): Promise<string> {
