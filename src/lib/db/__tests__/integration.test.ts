...
describe("Database Integration Tests", () => {
	...
	describe("Property Operations", () => {
		...
		it("should delete property successfully", async () => {
			...
			const result = await db.delete(properties).where({ id: "prop-123" });
	...
	describe("User Operations", () => {
		...
		it("should insert user successfully", async () => {
			...
			const result = await db.insert(users).values(mockUser).returning();
	...
	describe("Complex Queries", () => {
		...
		it("should handle aggregation queries", async () => {
			const mockSelect = {
				...
				having: jest
					.fn()
					.mockResolvedValue([{ userId: "user-123", propertyCount: 5 }]),
			};
	...
	describe("Performance and Optimization", () => {
		it("should handle large result sets efficiently", async () => {
			...
			const result = await db.select().from(properties).limit(1000);
