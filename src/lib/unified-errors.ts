...
export class NotFoundError extends AppError {
    constructor(
        ...
        super(`${resource} with identifier ${identifier} not found`, 'NOT_FOUND', 404, context);
...
export class ExternalServiceError extends AppError {
    constructor(
        ...
        super(message, `EXTERNAL_SERVICE_${service.toUpperCase()}`, 502, context, cause);
...
export function formatErrorResponse(error: unknown): {
    ...
    return {
        ...
        details: appError instanceof ValidationError ? appError.validationErrors : undefined
    };
}
...
export function validatePropertyBusinessRules(data: {
    ...
    if (data.type.toLowerCase().includes('house') || data.type.toLowerCase().includes('apartment')) {
        if (data.bedrooms < 1) {
