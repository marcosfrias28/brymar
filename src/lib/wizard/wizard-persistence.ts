...
export class WizardPersistence {
	...
	static async saveDraft<T extends WizardData>(
		...
		const finalDraftId =
			draftId || WizardPersistence.generateDraftId(wizardType, userId);
		const stepProgress = WizardPersistence.calculateStepProgress(
			data,
			configId,
		);
		const _completionPercentage =
			WizardPersistence.calculateCompletionPercentage(data, stepProgress);

		const draftData: Partial<NewWizardDraft> = {
			id: finalDraftId,
			userId: userId || "anonymous",
			type: wizardType as "property" | "land" | "blog",
			status: "draft",
			data: data as Record<string, any>,
			currentStep:
				typeof currentStep === "string"
					? parseInt(currentStep, 10) || 0
					: currentStep,
			steps: [],
			title: WizardPersistence.extractTitle(data),
			description: WizardPersistence.extractDescription(data),
			updatedAt: new Date(),
		};
		...
		if (userId && typeof window !== "undefined" && navigator.onLine) {
			try {
				const result = await WizardPersistence.saveToDatabaseWithRetry(
					draftData,
					!!draftId,
				);
				if (result.success) {
					// Update memory cache
					WizardPersistence.memoryCache.set(
						finalDraftId,
						result.data as WizardDraftWithMedia,
					);

					// Clear local storage backup if database save succeeded
					if (options.enableOfflineSupport !== false) {
						WizardPersistence.clearLocalStorageBackup(finalDraftId);
					}
				...
				console.warn(
					"Database save failed, falling back to local storage:",
					error,
				);
			}
		}
		...
		try {
			await WizardPersistence.saveToLocalStorageWithCompression(
				finalDraftId,
				draftData,
			);
			...
			WizardPersistence.memoryCache.set(
				finalDraftId,
				draftData as WizardDraftWithMedia,
			);
	...
	> {
		...
		const cachedDraft = WizardPersistence.memoryCache.get(draftId);
		if (cachedDraft) {
			return {
				success: true,
				data: {
					data: cachedDraft.data as Partial<T>,
					currentStep: String(cachedDraft.currentStep || 0),
					stepProgress: {},
		...
		if (userId && typeof window !== "undefined" && navigator.onLine) {
			try {
				const result = await WizardPersistence.loadFromDatabaseWithMedia(
					draftId,
					userId,
				);
				if (result.success && result.data) {
					// Update memory cache
					WizardPersistence.memoryCache.set(draftId, result.data);

					return {
						success: true,
						data: {
							data: result.data.data as Partial<T>,
							currentStep: String(result.data.currentStep || 0),
							stepProgress: {},
							completionPercentage: 0,
						},
						source: "database",
		...
		try {
			const draft =
				await WizardPersistence.loadFromLocalStorageWithDecompression(draftId);
			if (draft) {
				// Update memory cache
				WizardPersistence.memoryCache.set(draftId, draft);

				return {
					success: true,
					data: {
						data: draft.data as Partial<T>,
						currentStep: String(draft.currentStep || 0),
						stepProgress: {},
						completionPercentage: 0,
					},
					source: "localStorage",
	...
	static async deleteDraft(
		...
		WizardPersistence.memoryCache.delete(draftId);

		// Delete from database if user is authenticated
		if (userId && typeof window !== "undefined" && navigator.onLine) {
			try {
				const result = await WizardPersistence.deleteFromDatabaseWithMedia(
					draftId,
					userId,
				);
				results.push({
		...
		try {
			WizardPersistence.deleteFromLocalStorage(draftId);
		...
		const existingTimeout = WizardPersistence.autoSaveTimeouts.get(timeoutKey);
		if (existingTimeout) {
			clearTimeout(existingTimeout);
			WizardPersistence.autoSaveTimeouts.delete(timeoutKey);
		}
	...
	static autoSaveDraft<T extends WizardData>(
		...
		const interval = options.interval || WizardPersistence.AUTO_SAVE_DEBOUNCE;
		const key = draftId
			? `${draftId}_autosave`
			: `${wizardType}_${userId || "anonymous"}_autosave`;

		// Clear existing timeout
		const existingTimeout = WizardPersistence.autoSaveTimeouts.get(key);
		if (existingTimeout) {
			clearTimeout(existingTimeout);
		}
		...
		return new Promise((resolve) => {
			const timeout = setTimeout(async () => {
				try {
					...
					WizardPersistence.autoSaveTimeouts.delete(key);
					resolve(result);
				} catch (error) {
					WizardPersistence.autoSaveTimeouts.delete(key);
					resolve({
			...
			WizardPersistence.autoSaveTimeouts.set(key, timeout);
		});
	}
	...
	static async listDrafts(
		...
		if (typeof window !== "undefined" && navigator.onLine) {
			try {
				const result = await WizardPersistence.listDraftsFromDatabase(
					userId,
					options,
				);
				if (result.success) {
		...
		try {
			...
			for (const [_draftId, draft] of WizardPersistence.memoryCache.entries()) {
				if (draft.userId === userId) {
					if (!wizardType || draft.type === wizardType) {
						drafts.push(draft);
					}
				}
			}
			...
			if (typeof window !== "undefined" && window.localStorage) {
				for (let i = 0; i < localStorage.length; i++) {
					...
					if (key?.startsWith(WizardPersistence.STORAGE_PREFIX)) {
						const draftId = key.replace(WizardPersistence.STORAGE_PREFIX, "");
						if (!existingIds.has(draftId)) {
							const draft =
								await WizardPersistence.loadFromLocalStorageWithDecompression(
									draftId,
								);
			...
			drafts.sort((a, b) => {
				if (sortBy === "updatedAt" || sortBy === "createdAt") {
					...
					// For completionPercentage, we don't have it in the schema, so just use 0
					return 0;
				}
			});
	...
	static async hasDraft(
		...
		const result = await WizardPersistence.loadDraft(draftId, userId);
		return {
			success: true,
			data: result.success,
	 ...
	 */
	static async syncDrafts(
		userId: string,
	): Promise<PersistenceResult<{ synced: number; failed: number }>> {
		if (!userId || typeof window === "undefined" || !navigator.onLine) {
			return {
				success: false,
			...
			for (const draft of localDrafts) {
				try {
					...
					if (result.success) {
						...
						WizardPersistence.clearLocalStorageBackup(draft.id);
					} else {
						failed++;
					}
				} catch (error) {
	...
	static clearAutoSaveTimeouts(): void {
		WizardPersistence.autoSaveTimeouts.forEach((timeout) =>
			clearTimeout(timeout),
		);
		WizardPersistence.autoSaveTimeouts.clear();
	}
	...
	static clearMemoryCache(): void {
		WizardPersistence.memoryCache.clear();
	}
	...
	static getCacheStats(): {
		...
		return {
			memoryCache: {
				size: WizardPersistence.memoryCache.size,
				keys: Array.from(WizardPersistence.memoryCache.keys()),
			},
			localStorage: {
			...
			autoSaveTimeouts: {
				count: WizardPersistence.autoSaveTimeouts.size,
				keys: Array.from(WizardPersistence.autoSaveTimeouts.keys()),
			},
		};
	}
	...
	private static calculateStepProgress<T extends WizardData>(
		...
		switch (configId) {
			case "property-wizard":
				...
				progress.location = !!(
					(data as any).address && (data as any).coordinates
				);
				progress.media = !!(
					(data as any).images && (data as any).images.length > 0
				);
				progress.preview =
					progress.general && progress.location && progress.media;
				break;
			...
			case "land-wizard":
				...
				progress.location = !!(
					(data as any).location || (data as any).coordinates
				);
				progress.media = !!(
					(data as any).images && (data as any).images.length > 0
				);
			...
			case "blog-wizard":
				...
				progress.seo = !!(
					(data as any).seoTitle && (data as any).seoDescription
				);
	...
	private static extractTitle<T extends WizardData>(
		data: Partial<T>,
	): string | undefined {
		return data.title || (data as any).name || undefined;
	}

	private static extractDescription<T extends WizardData>(
		data: Partial<T>,
	): string | undefined {
		if (data.description) return data.description;
		if ((data as any).content)
			return `${(data as any).content.substring(0, 200)}...`;
		return undefined;
	}
	...
	private static async saveToLocalStorageWithCompression(
		...
		try {
			const key = WizardPersistence.STORAGE_PREFIX + draftId;
			const data = JSON.stringify(draft);

			// Check size limits
			if (data.length > WizardPersistence.MAX_LOCAL_STORAGE_SIZE) {
				throw new Error("Draft too large for local storage");
			}

			// Simple compression for large data
			const compressedData =
				data.length > 10000 ? WizardPersistence.compressData(data) : data;
			...
			await WizardPersistence.cleanupOldLocalStorageDrafts();
			...
			try {
				...
				console.warn(
					"Failed to save to local storage after cleanup:",
					retryError,
				);
				throw retryError;
			}
	...
	private static async loadFromLocalStorageWithDecompression(
		draftId: string,
	): Promise<WizardDraftWithMedia | null> {
		if (typeof window === "undefined" || !window.localStorage) {
			return null;
		}
		...
		try {
			...
			if (meta) {
				...
				if (metadata.compressed) {
					data = WizardPersistence.decompressData(stored);
				}
			}
	...
	private static async cleanupOldLocalStorageDrafts(): Promise<void> {
		...
		try {
			...
			for (let i = 0; i < localStorage.length; i++) {
				const key = localStorage.key(i);
				if (
					key?.startsWith(WizardPersistence.STORAGE_PREFIX) &&
					key.endsWith("_meta")
				) {
					try {
						...
						if (now - meta.timestamp > maxAge) {
							...
							draftsToRemove.push(
								draftKey.replace(WizardPersistence.STORAGE_PREFIX, ""),
							);
						}
					} catch (_error) {
	...
	private static async loadFromDatabaseWithMedia(
		...
		try {
			...
			const [draft] = await db
				...
				.from(wizardDrafts)
				.where(
					and(eq(wizardDrafts.id, draftId), eq(wizardDrafts.userId, userId)),
				);
			...
			return {
				...
				error:
					error instanceof Error
						? error.message
						: "Failed to load draft from database",
			};
		}
	}
		...
		try {
			...
			if (wizardType) {
				whereConditions.push(
					eq(wizardDrafts.type, wizardType as "property" | "land" | "blog"),
				);
			}
			...
			// Apply sorting - only support updatedAt and createdAt
			const sortColumn =
				sortBy === "completionPercentage"
					? wizardDrafts.updatedAt
					: wizardDrafts[sortBy];
			const queryWithSort =
				sortOrder === "desc"
			...
			return {
				...
				error:
					error instanceof Error
						? error.message
						: "Failed to list drafts from database",
			};
		}
	}
	...
	private static async deleteFromDatabaseWithMedia(
		...
		try {
			...
			await db.delete(wizardMedia).where(eq(wizardMedia.draftId, draftId));
