 ...
 */
export async function createLand(input: CreateLandInput): Promise<CreateLandResult> {
    try {
 ...
 */
export async function updateLand(input: UpdateLandInput): Promise<UpdateLandResult> {
    try {
        const session = await auth();
        if (!session?.user?.id) {
            return {
 ...
 */
export async function searchLands(filters: LandSearchFilters = {}): Promise<SearchLandsResult> {
    try {
        ...
        const availableFilters = {
            landTypes: ["residential", "commercial", "agricultural", "industrial", "recreational", "mixed-use", "vacant"] as LandType[],
            priceRanges: [
