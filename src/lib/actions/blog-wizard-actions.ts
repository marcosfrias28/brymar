...
export const createBlogFromWizard = async (data: BlogWizardData): Promise<ActionResult<{ blogPost: any }>> => {
    try {
...
export const updateBlogFromWizard = async (data: BlogWizardData & { id?: string }): Promise<ActionResult<{ blogPost: any }>> => {
    try {
        if (!data.id) {
...
// Save blog draft
export const saveBlogDraft = async (data: Partial<BlogWizardData> & {
    draftId?: string;
    userId: string;
...
// Upload blog image
export const uploadBlogImage = async (data: { title: string; image: File }): Promise<ActionResult<{ url: string; filename: string }>> => {
    try {
...
export const generateBlogContent = async (data: {
    title: string;
    category: string;
    contentType: "title" | "content" | "excerpt" | "seo-title" | "seo-description" | "tags";
    prompt?: string;
    ...
    try {
        ...
        switch (data.contentType) {
            ...
            case "seo-description":
                ...
                break;
            case "tags":
                generatedContent = "inmobiliaria,propiedades,inversión,mercado,consejos";
                break;
            default:
...
// Load blog draft
export const loadBlogDraft = async (data: { draftId: string; userId: string }): Promise<ActionResult<{ formData: Partial<BlogWizardData>; stepCompleted: number }>> => {
    try {
...
// Delete blog draft
export const deleteBlogDraft = async (data: { draftId: string; userId: string }): Promise<ActionResult> => {
    try {
...
export const generateSEOSuggestions = async (data: { title: string; content: string; category: string }): Promise<ActionResult<{
    seoTitle: string;
