    ...
    describe('updateBlogPost', () => {
        it('should update blog post successfully', async () => {
            ...
            const mockUpdate = {
                ...
                where: jest.fn().mockReturnThis(),
                returning: jest.fn().mockResolvedValue([{ ...mockBlogPost, title: 'Updated Blog Post' }])
            };
    ...
    describe('publishBlogPost', () => {
        it('should publish blog post successfully', async () => {
            ...
            const mockUpdate = {
                set: jest.fn().mockReturnThis(),
                where: jest.fn().mockReturnThis(),
                returning: jest.fn().mockResolvedValue([{ ...mockBlogPost, status: 'published' }])
            };
        ...
        it('should validate required fields for publishing', async () => {
            ...
            expect(result.success).toBe(false);
            expect(result.error).toBe('Blog post must have title and content to be published');
        });
    });
    ...
    describe('unpublishBlogPost', () => {
        it('should unpublish blog post successfully', async () => {
            ...
            const mockUpdate = {
                set: jest.fn().mockReturnThis(),
                where: jest.fn().mockReturnThis(),
                returning: jest.fn().mockResolvedValue([{ ...publishedPost, status: 'draft' }])
            };
