	...
	try {
		...
		switch (draft.type) {
			...
			default:
				return {
					success: false,
					error: `Unsupported wizard type: ${draft.type}`,
				};
		}
	...
	try {
		...
		switch (input.wizardType) {
			...
			default:
				return {
					success: false,
					error: `Unsupported wizard type: ${input.wizardType}`,
				};
		}
...
async function generatePropertyContent(
	...
	switch (contentType) {
		case "title":
			return {
				title: `Beautiful ${baseData.propertyType || "Property"} in ${
					baseData.location || "Prime Location"
				}`,
			};
		case "description":
			return {
				description: `This stunning ${
					baseData.propertyType || "property"
				} offers ${baseData.bedrooms || "multiple"} bedrooms and ${
					baseData.bathrooms || "modern"
				} bathrooms in the desirable ${
					baseData.location || "area"
...
async function generateLandContent(
	...
	switch (contentType) {
		case "title":
			return {
				title: `Prime ${baseData.landType || "Land"} - ${
					baseData.area || "Spacious"
		...
		case "description":
			return {
				...
				} opportunity covering ${baseData.area || "substantial area"} in ${
					baseData.location || "prime location"
...
async function generateBlogContent(
	...
	switch (contentType) {
		...
		case "description":
			return {
				description: `Discover everything you need to know about ${
					baseData.topic || "this topic"
