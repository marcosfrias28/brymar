...
function cleanGeneratedText(
	...
	if (
		originalPrompt.toLowerCase().includes("título") ||
		originalPrompt.toLowerCase().includes("title")
	) {
...
function generateTemplateDescription(
	...
	if (language === "en") {
		...
		const featuresText =
			characteristics.length > 0
				? ` Features include ${characteristics.slice(0, 3).join(", ")}.`
				: "";
	...
	const bedroomText = bedrooms
		? ` con ${bedrooms} habitacion${bedrooms > 1 ? "es" : ""}`
		: "";
	const bathroomText = bathrooms
		? ` y ${bathrooms} baño${bathrooms > 1 ? "s" : ""}`
		: "";
	const featuresText =
		characteristics.length > 0
			? ` Incluye ${characteristics.slice(0, 3).join(", ")}.`
			: "";
...
function _generateFallbackTitle(
	...
	if (language === "en") {
		...
		const bathroomText = bathrooms
			...
			: "";
		const details = [bedroomText, bathroomText, `${surface}m²`]
			.filter(Boolean)
			.join(", ");
...
function generateEnhancedFallbackDescription(
	...
	if (language === "en") {
		...
		const bathroomText = bathrooms
			...
			: "";
		const featuresText =
			characteristics.length > 0 ? characteristics.slice(0, 3).join(", ") : "";
