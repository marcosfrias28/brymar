...
export async function searchBlogPosts(
	...
	try {
		...
		return {
			success: false,
			error:
				error instanceof Error ? error.message : "Failed to search blog posts",
		};
	}
}
...
export async function publishBlogPost(
	...
	try {
		...
		if (existingPost[0].authorId !== session.user.id) {
			return {
				success: false,
				error: "Unauthorized to publish this blog post",
			};
		}
...
export async function unpublishBlogPost(
	...
	try {
		...
		if (existingPost[0].authorId !== session.user.id) {
			return {
				success: false,
				error: "Unauthorized to unpublish this blog post",
			};
		}
	...
	try {
		...
		return {
			...
			error:
				error instanceof Error
					? error.message
					: "Failed to get recent blog posts",
		};
	}
}
	...
	try {
		...
		return {
			...
			error:
				error instanceof Error
					? error.message
					: "Failed to get featured blog posts",
		};
	}
}
