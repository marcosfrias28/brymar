...
const listUnifiedDraftsSchema = z.object({
    ...
    sortBy: z.enum(["updatedAt", "createdAt", "completionPercentage"]).default("updatedAt"),
...
export async function deleteUnifiedDraft(
    ...
    try {
        ...
        return createSuccessResponse(
            undefined,
            "Borrador eliminado exitosamente"
        );
    } catch (error) {
...
export async function listUnifiedDrafts(
    ...
    try {
        ...
        const result = await WizardPersistence.listDrafts(
            validatedData.userId,
            {
 ...
 */
export async function getWizardCacheStats(): Promise<ActionState<{
