...
class CSRFTokenStore {
    ...
    get(sessionId: string): { token: string; expires: number; userId?: string } | undefined {
 ...
 */
export async function createCSRFToken(request: NextRequest, userId?: string): Promise<{
    token: string;
...
export async function validateCSRFToken(
    ...
    if (CSRF_CONFIG.EXEMPT_METHODS.includes(request.method as 'GET' | 'HEAD' | 'OPTIONS')) {
        return true;
    }
            ...
            return NextResponse.json(
                {
                    ...
                    message: 'Solicitud no válida. Por favor, recarga la página e inténtalo de nuevo.',
                },
                {
                    status: 403,
                    headers: {
 ...
 */
export async function getCSRFToken(request: NextRequest, userId?: string): Promise<string> {
    const sessionId = generateSessionId(request, userId);
    const storedData = csrfTokenStore.get(sessionId);
...
export class CSRFTokenManager {
     ...
     */
    async addTokenToJSON(data: Record<string, any>): Promise<Record<string, any>> {
        const token = await this.getToken();
        return {
            ...data,
...
export async function validateCSRFInServerAction(
    ...
    if (typeof token !== 'string' || token.length !== CSRF_CONFIG.TOKEN_LENGTH * 2) {
        throw new ValidationError(
