		...
		.map((char) => ({
			id:
				typeof char.id === "string"
					? sanitizeText(char.id, { maxLength: 50 })
					: "",
			name:
				typeof char.name === "string"
					? sanitizeText(char.name, {
							maxLength: SECURITY_CONFIG.MAX_LENGTHS.characteristic,
							allowedChars: SECURITY_CONFIG.ALLOWED_CHARS.alphanumeric,
						})
					: "",
			category: ["amenity", "feature", "location"].includes(char.category)
				? char.category
				: "feature",
				...
				if (Array.isArray(value)) {
					sanitized[key] = value
						...
						.map((img) => ({
							...
							displayOrder: sanitizeNumeric(img.displayOrder, {
								integer: true,
								min: 0,
							}),
						}))
