...
export class WizardPersistence {
    ...
    static createBackup(
        ...
        try {
            const history = this.loadWizardHistory();
            ...
            this.saveWizardHistory(history);
        } catch (error) {
    ...
    static getAvailableBackups(): Array<{
        ...
        try {
            ...
            return history.map((entry, index) => ({
                ...
                timestamp: entry.timestamp,
                action: entry.action,
                age: this.formatAge(now - entry.timestamp),
            }));
        } catch (error) {
    ...
    static exportWizardData(): string | null {
        ...
        try {
            const data = this.loadWizardData();
            const history = this.loadWizardHistory();
    ...
    static importWizardData(jsonData: string): boolean {
        ...
        try {
            ...
            if (importData.currentData) {
                localStorage.setItem(WIZARD_STORAGE_KEY, JSON.stringify(importData.currentData));
            }

            if (importData.history && Array.isArray(importData.history)) {
                this.saveWizardHistory(importData.history);
            }
