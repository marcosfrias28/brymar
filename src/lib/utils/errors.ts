...
// Error result helper
export function errorResult(error: string, errors?: Record<string, string[]>): ActionResult {
    return {
        success: false,
        error,
        errors,
...
export function validateVideoFile(file: File): void {
    ...
    if (!allowedTypes.includes(file.type)) {
        throw new FileUploadError("Invalid file type. Only MP4, WebM, and MOV are allowed.");
    }
