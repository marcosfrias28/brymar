import { PropertyTemplate, PropertyFormData, BulkImportData } from '@/types/template';
...
export class TemplateService {
    private static readonly DEFAULT_TEMPLATES: PropertyTemplate[] = [
        {
            ...
            defaultData: {
                ...
                description: 'Hermosa casa residencial con excelente ubicación y acabados de calidad.',
                surface: 150,
                bedrooms: 3,
                bathrooms: 2,
        ...
        },
        {
            ...
            defaultData: {
                ...
                description: 'Exclusiva villa de lujo con amenidades premium y vistas espectaculares.',
                surface: 400,
                bedrooms: 5,
                bathrooms: 4,
                characteristics: ['pool', 'ocean_view', 'gym', 'wine_cellar', 'smart_home'],
                language: 'es',
                status: 'draft'
            },
            characteristics: ['pool', 'ocean_view', 'gym', 'wine_cellar', 'smart_home', 'elevator', 'spa'],
            isActive: true,
            createdAt: new Date(),
            updatedAt: new Date()
        },
        {
            ...
            defaultData: {
                ...
                description: 'Moderno espacio comercial ideal para oficinas con excelente conectividad.',
                surface: 100,
                characteristics: ['air_conditioning', 'elevator', 'parking', 'security'],
                language: 'es',
                status: 'draft'
            },
            characteristics: ['air_conditioning', 'elevator', 'parking', 'security', 'conference_room'],
            isActive: true,
            createdAt: new Date(),
            updatedAt: new Date()
        },
        {
            ...
            defaultData: {
                ...
                description: 'Excelente terreno con potencial de desarrollo en ubicación privilegiada.',
                surface: 500,
                characteristics: ['utilities_ready', 'flat_terrain', 'road_access'],
                language: 'es',
                status: 'draft'
            },
            characteristics: ['utilities_ready', 'flat_terrain', 'road_access', 'corner_lot', 'development_potential'],
            isActive: true,
            createdAt: new Date(),
            updatedAt: new Date()
        }
    ...
    static getTemplates(): PropertyTemplate[] {
        return this.DEFAULT_TEMPLATES.filter(template => template.isActive);
    }

    static getTemplateById(id: string): PropertyTemplate | null {
        return this.DEFAULT_TEMPLATES.find(template => template.id === id) || null;
    }

    static getTemplatesByCategory(category: PropertyTemplate['category']): PropertyTemplate[] {
        return this.DEFAULT_TEMPLATES.filter(
            template => template.category === category && template.isActive
        );
    }

    static applyTemplate(template: PropertyTemplate, customData?: Partial<PropertyFormData>): PropertyFormData {
        const baseData = {
        ...
        if (customData) {
            return {
                ...
                ...customData,
                characteristics: [
                    ...new Set([...baseData.characteristics, ...(customData.characteristics || [])])
                ]
            } as PropertyFormData;
        }
    ...
    static validateBulkImportData(data: Array<Partial<PropertyFormData>>): BulkImportData['validationResults'] {
