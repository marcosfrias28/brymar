...
export class CharacteristicsService {
	...
	getCharacteristics(
		...
		return allCharacteristics
			.filter((char) => {
				...
				if (filter.propertyType) {
					...
					if (
						...
						!definition.propertyTypes.includes(filter.propertyType)
					) {
						return false;
					}
				}
			...
			.sort((a, b) => {
				...
				const aOrder =
					DEFAULT_CHARACTERISTICS.find((def) => def.id === a.id)?.order || 999;
				const bOrder =
					DEFAULT_CHARACTERISTICS.find((def) => def.id === b.id)?.order || 999;
	...
	updateTranslations(
		...
		this.characteristics.forEach((char, id) => {
			char.name =
				translations[id] ||
				DEFAULT_CHARACTERISTICS.find((def) => def.id === id)?.name ||
				char.name;
		});
	}
	...
	validateCharacteristicsForPropertyType(propertyType: PropertyType): {
		...
		selected.forEach((char) => {
			if (char.id.startsWith("custom_")) {
				...
				if (
					!definition?.propertyTypes ||
					definition.propertyTypes.includes(propertyType)
				) {
					valid.push(char);
				} else {
					invalid.push(char);
