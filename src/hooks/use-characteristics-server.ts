...
export interface UseCharacteristicsServerReturn {
    ...
    addCustomCharacteristic: (name: string, category?: "amenity" | "feature" | "location") => Promise<PropertyCharacteristic | null>;
    removeCustomCharacteristic: (id: string) => Promise<boolean>;
    saveCharacteristics: (draftId?: string) => Promise<void>;
    searchCharacteristics: (query: string, category?: "amenity" | "feature" | "location") => Promise<PropertyCharacteristic[]>;
...
export function useCharacteristicsServer(
    ...
    // Computed values
    const selectedCharacteristics = characteristics.filter(char => char.selected);
    const isValid = selectedCharacteristics.length > 0;
    const validationMessage = !isValid ? "Please select at least one characteristic" : undefined;
    ...
    const loadDraftCharacteristics = useCallback(async (targetDraftId: string) => {
        try {
            setIsLoading(true);
            const draftCharacteristics = await getDraftCharacteristics(targetDraftId);
            ...
            setCharacteristics(prev => {
                ...
                draftCharacteristics.forEach(draftChar => {
                    const existingIndex = merged.findIndex(char => char.id === draftChar.id);
                    if (existingIndex >= 0) {
                        merged[existingIndex] = { ...merged[existingIndex], selected: draftChar.selected };
                    } else {
    ...
    const setCharacteristicSelected = useCallback((id: string, selected: boolean) => {
        setCharacteristics(prev =>
            prev.map(char =>
                char.id === id ? { ...char, selected } : char
            )
        );
    }, []);

    // Add custom characteristic
    const addCustomCharacteristic = useCallback(async (
        name: string,
        category: "amenity" | "feature" | "location" = "feature"
    ): Promise<PropertyCharacteristic | null> => {
        try {
            const newCharacteristic = await createCustomCharacteristic(name, category);

            setCharacteristics(prev => [...prev, { ...newCharacteristic, selected: true }]);
    ...
    const removeCustomCharacteristic = useCallback(async (id: string): Promise<boolean> => {
        try {
    ...
    const saveCharacteristics = useCallback(async (targetDraftId?: string) => {
    ...
    const searchCharacteristicsAction = useCallback(async (
        query: string,
        category?: "amenity" | "feature" | "location"
    ): Promise<PropertyCharacteristic[]> => {
        try {
            setIsSearching(true);
            const results = await searchCharacteristics(query, propertyType, category);
            return results;
        } catch (error) {
    ...
    const getCharacteristicsByCategory = useCallback((): Record<string, PropertyCharacteristic[]> => {
    ...
    useEffect(() => {
        triggerAutoSave();
    }, [characteristics, triggerAutoSave]);
