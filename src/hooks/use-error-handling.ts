		...
		async <T>(
			...
			try {
				...
				if (operationOptions.queueIfOffline && !networkStatus.isOnline) {
					const _queueId = await queueOperation(operation, {
						type: operationOptions.queueType || "general",
						description:
							operationOptions.queueDescription || "Operación pendiente",
...
export function useErrorRecovery() {
	const handleAIError = useCallback(
		(error: AIServiceError, fallbackContent?: string) => {
			...
			return {
				shouldUseFallback:
					error.code === "RATE_LIMIT" || error.code === "API_ERROR",
				fallbackContent,
				recovery,
			};
		},
			...
			customActions: [
				{
					...
					primary:
						error.code === "INVALID_TYPE" || error.code === "FILE_TOO_LARGE",
				},
			],
		});
