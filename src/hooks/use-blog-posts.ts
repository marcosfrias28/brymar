 ...
 */
export function useBlogPostsByCategory(category: string, page: number = 1, limit: number = 12) {
    return useQuery({
 ...
 */
export function useBlogPostForm(initialData?: Partial<CreateBlogPostInput>) {
    const createMutation = useCreateBlogPost();
    const updateMutation = useUpdateBlogPost();

    const handleSubmit = async (data: CreateBlogPostInput | UpdateBlogPostInput) => {
        if ("id" in data) {
