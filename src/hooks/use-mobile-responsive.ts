...
export function useResponsive(options: UseResponsiveOptions = {}) {
    ...
    const getSpacing = useCallback((mobile: string, tablet: string, desktop: string) => {
        if (state.isMobile) return mobile;
        if (state.isTablet) return tablet;
        return desktop;
    }, [state.isMobile, state.isTablet]);
...
export function useMobilePerformance() {
    ...
    return {
        ...
        shouldLazyLoad: isLowEndDevice || connectionType === 'slow-2g' || connectionType === '2g',
    };
}
...
export function useMobileFormOptimization() {
    ...
    const getInputProps = useCallback((inputType: string = 'text') => {
        let inputMode: 'text' | 'numeric' | 'email' | 'tel' | 'url' = 'text';
        ...
        return {
            ...
            spellCheck: inputType === 'email' || inputType === 'number' ? false : true,
    ...
    const getTextareaProps = useCallback(() => {
        return {
            ...
            autoCorrect: 'on',
            spellCheck: true,
            className: isMobile ? 'text-base px-4 py-3 min-h-[120px]' : 'text-sm px-3 py-2 min-h-[100px]',
        };
    }, [isMobile]);
