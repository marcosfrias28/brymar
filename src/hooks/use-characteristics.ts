...
import { CharacteristicsService, CharacteristicsFilter } from '@/lib/services/characteristics-service';
...
export interface UseCharacteristicsReturn {
    ...
    addCustomCharacteristic: (name: string, category?: "amenity" | "feature" | "location") => PropertyCharacteristic;
...
export function useCharacteristics(options: UseCharacteristicsOptions = {}): UseCharacteristicsReturn {
    const {
        propertyType,
    ...
    const filter = useMemo((): CharacteristicsFilter => ({
        propertyType,
    ...
    const removeCustomCharacteristic = useCallback((id: string) => {
    ...
    const selectRecommendedForPropertyType = useCallback(() => {
        ...
        const recommendedByCategory = recommended.reduce((acc, char) => {
        ...
        setUpdateTrigger(prev => prev + 1);
    }, [propertyType, clearAllSelections]);

    const validateForPropertyType = useCallback((targetPropertyType: PropertyType) => {
...
export function useCharacteristicsForm(
    ...
    const importFromForm = useCallback((formCharacteristics: PropertyCharacteristic[]) => {
