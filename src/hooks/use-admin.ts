...
export function useAdmin() {
	...
	const hasPermission = (permission: Permission): boolean => {
		...
		const rolesForPermission =
			allowedRoles[permission as keyof typeof allowedRoles];
		return rolesForPermission
			? rolesForPermission.includes(userRole as any)
			: false;
	};
	...
	const canAccessRoute = (pathname: string): boolean => {
		...
		if (matchingRoute) {
			const requiredPermission =
				ROUTE_PERMISSIONS[matchingRoute as keyof typeof ROUTE_PERMISSIONS];
