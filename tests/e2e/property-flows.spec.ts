...
test.describe('Property Management Flows', () => {
    ...
    await page.evaluate(() => {
      ...
      localStorage.setItem('auth-session', JSON.stringify({
        user: { 
          ...
          role: 'admin',
          permissions: ['canManageProperties', 'canCreateProperties', 'canEditProperties']
        }
      }));
    });
  });
  ...
  test('should handle property creation flow (admin/agent)', async ({ page }) => {
    ...
    if (await descriptionInput.isVisible()) {
      await descriptionInput.fill('This is a test property created by E2E tests');
    }
    ...
    if (await fileInput.isVisible()) {
      ...
      if (hasMultiple !== null) {
        ...
        const multipleUploadIndicators = [
          ...
          page.locator('img').nth(1) // Second image preview
        ];
...
test.describe('Property Search and Filters', () => {
  ...
  test('should handle property type filtering', async ({ page }) => {
    ...
    if (typeFilter) {
      ...
      const propertyTypes = ['villa', 'residential', 'commercial', 'apartment', 'house'];
      ...
      for (const type of propertyTypes) {
        ...
        if (matchingOption) {
          ...
          const filterResults = [
            ...
            page.locator(`text=${type.charAt(0).toUpperCase() + type.slice(1)}`),
            page.locator('[data-testid="property-card"]'),
            page.locator('.property-card')
          ];
  ...
  test('should handle sorting options', async ({ page }) => {
    ...
    for (const element of sortElements) {
      if (await element.isVisible()) {
        if (element.tagName === 'SELECT') {
          const options = await element.locator('option').allTextContents();
          const sortOptions = options.filter(option => 
